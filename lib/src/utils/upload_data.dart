...
Future<List<SelectedFile>?> selectMediaWithSourceBottomSheet({
  ...
  required final bool allowPhoto,
  final String? storageFolderPath,
  final double? maxWidth,
  final double? maxHeight,
  final int? imageQuality,
  final bool allowVideo = false,
  final String pickerFontFamily = 'Roboto',
  final Color textColor = const Color(0xFF111417),
  final Color backgroundColor = const Color(0xFFF5F5F5),
  final bool includeDimensions = false,
  final bool includeBlurHash = false,
}) async {
  ListTile createUploadMediaListTile(
    final String label,
...
Future<List<SelectedFile>?> selectMedia({
  ...
  final MediaSource mediaSource = MediaSource.camera,
  final bool multiImage = false,
  final bool includeDimensions = false,
  final bool includeBlurHash = false,
}) async {
  ...
  if (multiImage) {
    ...
    return Future.wait(
      pickedMedia.asMap().entries.map((final e) async {
        final index = e.key;
        final media = e.value;
...
bool validateFileFormat(final String filePath, final BuildContext context) {
...
Future<List<SelectedFile>?> selectFiles({
  ...
  if (multiFile) {
    return Future.wait(
      pickedFiles.files.asMap().entries.map((final e) async {
        final index = e.key;
        final file = e.value;
        final storagePath =
...
List<SelectedFile> selectedFilesFromUploadedFiles(
  final List<FFUploadedFile> uploadedFiles, {
  final String? storageFolderPath,
  final bool isMultiData = false,
}) =>
...
Future<MediaDimensions> _getImageDimensions(final Uint8List mediaBytes) async {
...
Future<MediaDimensions> _getVideoDimensions(final String path) async {
...
String _getStoragePath(
  final String? pathPrefixString,
  final String filePath,
  final bool isVideo, [
  final int? index,
]) {
  final pathPrefix = _removeTrailingSlash(pathPrefixString);
                ...
                child: CircularProgressIndicator(
                  valueColor: Theme.of(context).brightness == Brightness.dark
                      ? const AlwaysStoppedAnimation<Color>(
                          Color(0xFF2196F3), // TODOmake this a theme color
                        )
                      : null,
                ),
              ),
...
String? _removeTrailingSlash(final String? path) =>
